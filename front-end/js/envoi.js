    ...
    data.forEach(entry => {
        if (weight >= entry.minWeight && weight <= entry.maxWeight) {
            ...
            rowColis.innerHTML = `
                ...
                <td><button class="choose-button">Choisir</button></td>
            `;
...
document.getElementById('cinSearchButton').addEventListener('click', function() {
    ...
    if (database[cin]) {
        document.getElementById('receiverCIN').value = database[cin].cin;
        document.getElementById('receiverFirstName').value = database[cin].firstName;
        document.getElementById('receiverLastName').value = database[cin].lastName;
        document.getElementById('receiverPhoneNumber').value = database[cin].phoneNumber;
    } else {
        document.getElementById('receiverCIN').value = '';
...
document.getElementById('submitButton').addEventListener('click', function() {
    ...
    if (!senderCIN || !senderFirstName || !senderLastName || !senderPhoneNumber || !productDestination || !productType || !receiverCIN || !receiverFirstName || !receiverLastName || !receiverPhoneNumber) {
        alert('Veuillez remplir tous les champs.');
        return;
    }

    document.getElementById('receiptSenderCIN').innerText = senderCIN;
    document.getElementById('receiptSenderName').innerText = `${senderFirstName} ${senderLastName}`;
    ...
    document.getElementById('receiptReceiverName').innerText = `${receiverFirstName} ${receiverLastName}`;
    document.getElementById('receiptReceiverPhone').innerText = receiverPhoneNumber;
...
document.addEventListener('click', function(event) {
    if (event.target.tagName === 'BUTTON' && event.target.classList.contains('choose-button')) {
        document.querySelectorAll('td button').forEach(btn => btn.classList.remove('selected'));
